...
const ViewPost = () => {
    ...
    return (
        <div className="w-screen h-screen flex items-center justify-start flex-col">
           ...
           {post.map((post) => {
           return (
           <div key={post._id} className='w-3/4 mb-8'>
                <div className="mt-6 border-t border-gray-100 px-8 mb-8">
                    ...
                    <div className='w-3/4 pl-8'>
                        ...
                        <Link to={`/api/application/${post._id}`}><button type="button" className="transition ease-in-out delay-100 inline-flex items-center rounded-md bg-[#f1c40faf] px-3 py-2 text-sm text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-[#f1c40f] hover:-translate-y-1 hover:scale-110 mb-12">Edit Application</button></Link>
                    </div>
            </div>
            )
            })}
